...
const NextArrow: React.FC<ArrowProps> = ({ onClick }) => (
  ...
  >
    <FaChevronRight className="text-orange-400 text-[44px]" />
  </div>
);
...
const PrevArrow: React.FC<ArrowProps> = ({ onClick }) => (
  ...
  >
    <FaChevronLeft className="text-orange-400 text-[44px]" />
  </div>
);
...
const Testimonials: React.FC = () => {
  ...
  return (
    <div className="py-10 bg-black border-t-2 border-orange-400">
        ...
        <Slider {...settings}>
          {testimonials.map((testimonial) => (
            <div key={testimonial.id} className="p-6">
              <div className="relative bg-transparent p-8 ring-2 ring-orange-400 rounded-lg shadow-md flex flex-col justify-center items-center text-center">
                {/* Testimonial Text */}
