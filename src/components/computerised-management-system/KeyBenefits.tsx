    ...
    <div className="bg-black border-b-2 border-green-600">
      <div className="container mx-auto py-12 px-4">
        <div className="flex justify-center items-center">
          <h2 className="text-4xl text-green-600 mb-4">Key Benefits</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="bg-transparent rounded-md border-[1px] flex flex-col justify-center items-center border-green-600 p-4"
            >
              <img src={benefit.img} alt={benefit.title} />
              <p className="text-center text-white">{benefit.title}</p>
            </div>
          ))}
        </div>
      </div>
